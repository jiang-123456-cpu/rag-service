...
def get_file_documents(read_path: str):
    ...
    if read_path.endswith("docx"):
        try:
            ...
            # 4. 文本框（通过底层XML提取）
            try:
                textboxes = doc.element.xpath('//w:txbxContent')
                for tb in textboxes:
                    for para in tb.xpath('.//w:p'):
                        text = ''.join(node.text for node in para.xpath('.//w:t') if node.text)
                        if text.strip():
            ...
            if not content.strip():
                # 抛出异常，让上层捕获并显示明确原因
                raise Exception("DOCX 中未检测到任何文本内容（可能为扫描件/图片型或仅包含无文字对象）")
