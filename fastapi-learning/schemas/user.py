...
class ChangePasswordUser(BaseModel):
    ...
    newpassword: Annotated[
        ...
        Field(
            min_length=6,
            max_length=12,
            description="新密码6-12位，必须同时包含英文和数字，仅允许英文、数字",
            examples=["Abc12345", "Pass678"],
        )
    ]
    confirm_password: Annotated[
        ...
        Field(
            min_length=6,
            max_length=12,
            description="确认密码6-12位，必须同时包含英文和数字，仅允许英文、数字",
            examples=["Abc12345", "Pass678"],
        )
    ]
    @model_validator(mode='after')
    def check_confirm_password(self):
        if self.confirm_password != self.newpassword:
    ...
    def validate_new_password(cls, v):
        if not re.match(r"^[a-zA-Z0-9]+$", v):
            raise ValueError("新密码只能包含英文字母和数字")
        if not re.search(r"[a-zA-Z]", v):
            raise ValueError("新密码必须包含至少一个英文字母")
        if not re.search(r"\d", v):
